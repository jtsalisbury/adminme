...
function PANEL:SetPlaceholder(text)
	self.pHolderTxt = text
end

function PANEL:SetTheme(type)
	self.theme = type
end

function PANEL:Paint( w, h )
	local col = cols.ctrl_text_entry
	local bord = cols.ctrl_text_entry

	if (self.theme == "LIGHT") then
		col = Color(255, 255, 255)
		bord = Color(230, 230, 230)
	end
	...
	self:DrawTextEntryText(cols.ctrl_text, cols.ctrl_text_highlight, Color(0, 0, 0))
...
derma.DefineControl( "am.DTextEntry", "A simple TextEntry control", PANEL, "TextEntry" )
